...
| `tracking.events` | Tracking orchestrator, CTS | `event`: raw `TrackingEvent` protobuf | CC tracking and world-observation subscribers | Add decision ID, inferred and effective identity, authority, source, conflict, calibrated confidence, and evidence reference | One coordinated deployment after canonical fields are consumed |
...
| `cc.identity_assertions` | Cognitive Companion | Current exception: separate text fields for person, score, camera, time, and floor point | CTS assertion subscriber and face identity stage | One protobuf field with source, raw similarity, nullable calibrated confidence, model versions, and calibration status | Zero-period producer and consumer cutover |
