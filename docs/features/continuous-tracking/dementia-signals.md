...
A periodic worker (`DementiaSignalWorker`) runs every 60 s (configurable) and computes nine signal kinds per tracked identity. Two of them, `fall_suspected` and `agitation_index`, are opt-in per deployment and default to off. Each signal is persisted via upsert with a deterministic UUID5 so the same detection window always maps to the same row.
...
| `fall_suspected` | Sudden vertical collapse followed by prolonged floor-level stillness, detected from RTMPose keypoints and Kalman floor velocity. The fast-path detector fires within ~3 s of impact. A VLM confirmation step runs once per incident and its verdict is appended to the caregiver notification. Gated by `fall_detection.enabled` (default: off); enable per-deployment after reviewing the fixture proofs. **Fall detection is supportive and is not a substitute for a medical alert system. Do not rely on it as your sole fall-response mechanism.** | No statistical baseline: deterministic rule thresholds on vertical-collapse rate, height-ratio drop, and post-event motion energy. Thresholds are calibrated from replay fixtures and documented in `continuous-tracking/config/settings.yaml` under `fall_detection:`. | Starts at `warning` (fall suspected); escalates to `emergency` on confirmed post-event stillness exceeding the lie threshold. |
| `gait_slowing` | Sustained gait speed decline over a 56-day two-window comparison: duration-weighted median walking speed for the most recent 28 days vs the prior 28 days. Fires only when the decline is both absolute (>= max(0.08 m/s, 10% of the baseline median)) and statistically robust (modified z <= -2.0). Requires 10+ qualifying days in each window; a day qualifies with 3+ walking bouts and 60+ s of total walking. The current partial day is excluded. Per-kind cooldown of 7 days, so a monthly trend does not re-page daily. | Daily gait records (per-day median speed, bout count, walking seconds) from the gait daily repository over 56 days. The prior 28 days form the baseline; the robust z compares the recent weighted median against the baseline daily medians. | `info` on trigger; `warning` when the decline exceeds 20% or the recent median falls below 0.6 m/s. |
| `agitation_index` | In-place restlessness over the last 30 min: a composite [0, 1] index of 0.5 x in-place motion ratio + 0.3 x direction change entropy + 0.2 x normalized short-excursion rate (entropy and excursion terms contribute 0 on uncalibrated cameras). Fires when the composite exceeds the configured threshold and the modified z against the personal baseline exceeds the z gate. Gated by `agitation_enabled` (default: off). Experimental: no cold-start fallback, so it stays silent until enough baseline windows accumulate, and its context payload carries an explicit no-clinical-validation disclaimer. | Prior composite windows persisted to `agitation_windows` after every evaluation, excluding the current window. Requires `min_baseline_n` samples before it can emit at all. | `info` on trigger; `warning` when both the composite and the modified z exceed their warning gates. |
